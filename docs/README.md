...
- **Adding a second trigger for an existing state re-opens every guard the first trigger closed** (edge detection, cooldowns, re-arm flags). A guarded chat trigger plus an unguarded on-screen-marker trigger for the same state produced a five-minute enter/fail/re-enter loop that ended in a `WL.Activity` shutdown. See `script-anatomy.md` section 4.
- **Sanity-check `Map.Position()` against plausible movement speed before acting on it.** It can lock onto the wrong match in repetitive terrain; one false reading fed to `WebWalk` pathfound from a nonexistent start point and dragged the bot 119 tiles out of the area. See `map-walking.md` section 7.
